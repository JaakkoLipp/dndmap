...
async def create_layer(
    ...
    event_payload = {"layer_id": str(layer.id), "name": layer.name}
    await publish_event(
        request,
        map_id,
    ...
    await write_revision(
        db,
        map_id=map_id,
        event_type=EventType.LAYER_CREATED,
        actor=user,
...
async def update_layer(
    ...
    updated = await store.update_layer(layer_id, changes)
    if updated is None:
        raise_not_found("Layer")
    event_payload = {
    ...
    await write_revision(
        ...
        event_type=EventType.LAYER_UPDATED,
        actor=user,
        summary=f"Updated layer \"{updated.name}\"",
        payload=event_payload,
    )
    return updated
...
async def delete_layer(
    ...
    if not await store.delete_layer(layer_id):
        raise_not_found("Layer")
    event_payload = {"layer_id": str(layer_id), "name": layer_name}
    await publish_event(
        request,
        map_id,
    ...
    await write_revision(
        db,
        map_id=map_id,
        event_type=EventType.LAYER_DELETED,
        actor=user,
